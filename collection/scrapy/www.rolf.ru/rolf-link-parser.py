...
class RolfSpider(scrapy.Spider):
    name = 'rolf-link-parser'
    allowed_domains = ['www.rolf.ru']
    start_urls = ["https://www.rolf.ru/cars/new/?order_by=name&dir=desc"]

    def parse(self, response):
        count = "".join(response.css(".btn-extra-filter-left > span:nth-child(1) > span:nth-child(1)::text").extract_first().split(" "))
        ...
        for i in range(1, int(count)+1):
            ...
            yield scrapy.FormRequest('https://www.rolf.ru/ajax/stock/load_more/', headers=headers, callback=self.parse_link,
                                     method='POST', formdata=data)

    def parse_link(self, response):
        for response in response.css("body"):
            links = response.css("a::attr(href)").extract()
            for id, value in enumerate(links):
                links[id] = str(links[id]).replace("\\", "")
                links[id] = links[id].replace("\"", "")
                if (links[id] == "https://www.rolf.ru/agreement/"):
                    del links[id]
                else:
                    links[id] = "https://www.rolf.ru" + links[id]
            link = "".join(links)
            if (link != ""):
                yield Request(link, callback=self.parse_car)
                # yield Request("https://www.rolf.ru/cars/new/skoda/karoq/stock_car527878/", callback=self.parse_car)
    ...
    def parse_car(self, response):
        ...
        for response in response.css("body"):
            ...
            if (data['Специальная цена'] == "₽"):
                data['Специальная цена'] = ""
                try:
                    data['Стоимость автомобиля'] = "".join(response.css("div.car-page__price-block:nth-child(1) > p:nth-child(2)::text").extract_first().split(" "))
                except Exception:
                    data['Стоимость автомобиля'] = ""
            ...
            if (data['imgs'] == ""):
                try:
                    data['imgs'] = response.css("div.image_gallery > img::attr(data-image-preload)").extract_first()
                    data['imgs'] = "https://www.rolf.ru" + data['imgs']
                except Exception:
                    data['imgs'] = ""
            ...
            try:
                color_links = response.css("a.car-page__colors-item::attr(href)").extract()
                colors = response.css("a.car-page__colors-item > div::attr(style)").extract()
                out = ""
                for id, value in enumerate(color_links):
                    color_links[id] = "https://www.rolf.ru" + color_links[id]
                    colors[id] = colors[id].split("background: ", 1)[1]
                    colors[id] = colors[id].split(";", 1)[0]
                    out += colors[id] + " " + color_links[id] + ","
                out = set(out.split(","))
                out = list(filter(None, out))
                data['Доступные цвета'] = ",".join(out)
            except Exception:
                data['Доступные цвета'] = ""
            try:
                data['Ссылка на салон'] = response.css(".image_salon-logo::attr(href)").extract_first()
                data['Ссылка на салон'] = "https://www.rolf.ru" + data['Ссылка на салон']
            except Exception:
                data['Ссылка на салон'] = ""
            yield data
